...
public class AddTodo extends JFrame {
	...
	public AddTodo() throws ParseException {
		setTitle("To Do List");
		...
		completecomboBox.setModel(new DefaultComboBoxModel<Object>(new String[] { "X" }));
		...
		percentcomboBox.setModel(new DefaultComboBoxModel<Object>(new String[] { "0%" }));
		...
		panel.add(okbutton);
		if (percentcomboBox.getSelectedItem() == "100%")
			completecomboBox.setSelectedItem("O");
		else
			completecomboBox.setSelectedItem("X");
					...
					try {
						...
						if (e.getSource().equals(okbutton)) {
							...
							if ((textField.getText().isEmpty()) == true || (textField_2.getText().isEmpty()) == true) {
								...
								sql = "insert into Todo values('" + textField_2.getText() + "','" + textField.getText()
										+ "','" + deadlinecomboBox.getSelectedItem() + "','"
										+ realdeadlinecomboBox.getSelectedItem() + "','"
										+ importancecomboBox.getSelectedItem() + "','"
										+ completecomboBox.getSelectedItem() + "','" + percentcomboBox.getSelectedItem()
										+ "');";

								stmt.executeUpdate(sql);
								JOptionPane.showMessageDialog(null, "할일 등록이 완료 되었습니다.");
								setVisible(false);
